...
running, lost, start_state, particle_state = True, False, False, True
player_vel, meteor_vel = 3, 6
fps = 120
meteor_list, particle_list = [], [] 
bg_y = 0
...
pygame.display.set_caption("Dodge-ship")
...
class Particles:
    ...
    def draw(self):
        if self.time < max_time/2 * fps:
            self.color = (255, 0, 0)
        if self.time > max_time/2 * fps:
            self.color = (226, 88, 34)
        self.time += 1
        particle_h = random.randrange(2, 10)
...
while running:
    ...
    if start_state == False:
        ...
        while len(particle_list) < 20:
            rate += 4
            particle = Particles(random.randrange((player.x), (player.x + 50)) , player.y + rate + player_h)
