...
def main():
    ...
    def redraw_window():
        ...
        if lost == True:
            lost_lable = lost_font.render("You Ded!", 1, (255, 255, 255))
            win.blit(lost_lable, (w//2 - lost_lable.get_width()/2, 350))
    ...
    while run:
        ...
        if len(enemies) == 0:
            ...
            for i in range(wave_length):
                enemy = Enemy(random.randrange(50, w - 100), random.randrange((h - 1000)*2, h - 800), random.choice(["red", "blue", "green"]))
...
def main_menu():
    ...
    while run:
        ...
        win.blit(title_lable, (w/2 - title_lable.get_width()/2, 350))
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
